...
export function getAllProgramIdls() {
  return getAllProgramFolders().map((folder) => path.join(workingDirectory, folder, 'idl.json'))
}
...
export function getProgramFolders() {
  ...
  const filteredPrograms = programs.filter((program) => fs.existsSync(path.join(workingDirectory, program)))

  if (filteredPrograms.length !== programs.length && !didWarnAboutMissingPrograms) {
...
export function getCargo(folder) {
  return parseToml(fs.readFileSync(path.join(workingDirectory, folder ? folder : '.', 'Cargo.toml'), 'utf8'))
}
...
export function partitionArguments(args, delimiter) {
  ...
  return index >= 0 ? [args.slice(0, index), args.slice(index + 1)] : [args, []]
}
